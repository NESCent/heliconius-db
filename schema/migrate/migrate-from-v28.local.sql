...
--
-- This is a special version that takes into account the local changes
-- to the geolocation table. DO NOT use this script unless you are
-- certain that it applies to your database instance. Otherwise, use
-- the script named identically except for not having 'local' in the
-- name.

-- 1) Migrate politlocation
ALTER TABLE geolocation ADD COLUMN postalcode character varying(32);
-- In the locally changed version these three columns have already been added:
...
-- So let's just make sure their column type is standard:
...
-- In the locally changed version this won't have any effect:
